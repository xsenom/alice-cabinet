...
type VideoItem = { id: string; title: string; hint: string; free: boolean };
...
export default function HomePage() {
    ...
    const videos = useMemo<VideoItem[]>(
        () => [
            { id: "v1", title: "Как пользоваться Lesik", hint: "60 секунд: трафик → бот → воронка → оплата", free: true },
            { id: "v2", title: "Кому будет полезно", hint: "5 кейсов: эксперты, школы, мастера, сервисы", free: true },
            { id: "v3", title: "Mini App в Telegram", hint: "Каталог / квиз / кабинет / оплата — быстро", free: false },
        ],
        []
    );
                            ...
                            return (
                                <div key={v.id} className="snap-start">
                                    <button
                                        ...
                                        onClick={() => {
                                            if (locked) return;
                                            nav(`/library?open=${encodeURIComponent(v.id)}`);
                                        }}
                                        className="text-left"
                                    >
