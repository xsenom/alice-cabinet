...
export function useSessionProfile(): State {
    ...
    const load = async (): Promise<void> => {
        ...
        const promise = (async () => {
            ...
            const { data: prof, error: profErr } = await supabase
                .from("profiles_les")
                .select(
                    "id,email,original_email,full_name,profession,avatar_url,status_admin,plan_status,plan_expires_at,created_at,updated_at"
                )
                .eq("id", u.id)
                .maybeSingle();
            ...
            if (prof.email !== (u.email ?? null)) {
                await supabase.from("profiles_les").update({ email: u.email ?? null }).eq("id", u.id);
            }

            if (!prof.original_email && u.email) {
                await supabase.from("profiles_les").update({ original_email: u.email }).eq("id", u.id);
            }
            ...
            const normalizedProfile: CabinetProfile = {
                ...
                status_admin: !!prof.status_admin,
    ...
    return useMemo(
        () => ({ loading, user, profile, error, refresh }),
